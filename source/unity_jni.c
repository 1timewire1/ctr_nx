...
/* ZFramework's current Java wrapper is obfuscated as bk/bk/bl.  Its methods
 * delegate one-for-one to ZPreferences, so expose the same persistent KV store
 * to jni_fake.c instead of maintaining a second, incompatible save database. */
int unity_prefs_has_key(const char *key){
  return key&&key[0]&&kv_get(key)!=NULL;
}
int64_t unity_prefs_get_int64(const char *key,int64_t fallback){
  KV *kv=key?kv_get(key):NULL;
  return kv?(int64_t)strtoll(kv->val,NULL,10):fallback;
}
float unity_prefs_get_float(const char *key,float fallback){
  KV *kv=key?kv_get(key):NULL;
  return kv?(float)strtod(kv->val,NULL):fallback;
}
int unity_prefs_get_bool(const char *key,int fallback){
  KV *kv=key?kv_get(key):NULL;
  if(!kv)return fallback!=0;
  return kv->val[0]=='1'||kv->val[0]=='t'||kv->val[0]=='T';
}
const char *unity_prefs_get_string(const char *key,const char *fallback){
  KV *kv=key?kv_get(key):NULL;
  return kv?kv->val:(fallback?fallback:"");
}
void unity_prefs_set_int64(const char *key,int64_t value){
  if(!key||!key[0])return;
  char text[32];snprintf(text,sizeof text,"%lld",(long long)value);kv_set('L',key,text);
}
void unity_prefs_set_int(const char *key,int value){
  if(!key||!key[0])return;
  char text[32];snprintf(text,sizeof text,"%d",value);kv_set('I',key,text);
}
void unity_prefs_set_float(const char *key,float value){
  if(!key||!key[0])return;
  char text[32];snprintf(text,sizeof text,"%.9g",(double)value);kv_set('F',key,text);
}
void unity_prefs_set_bool(const char *key,int value){
  if(key&&key[0])kv_set('B',key,value?"1":"0");
}
void unity_prefs_set_string(const char *key,const char *value){
  if(key&&key[0])kv_set('S',key,value?value:"");
}
void unity_prefs_remove(const char *key){
  if(key&&key[0])kv_remove(key);
}
void unity_prefs_remove_prefix(const char *prefix){
  if(!prefix)return;
  const size_t length=strlen(prefix);
  for(int i=0;i<g_kv_n;){
    if(!strncmp(g_kv[i].key,prefix,length)){
      free(g_kv[i].key);free(g_kv[i].val);g_kv[i]=g_kv[--g_kv_n];g_kv_dirty=1;
    }else i++;
  }
}
void unity_prefs_clear(void){kv_clear();}
...
void *unity_dispatch_object(void *recv, const void *id_, va_list va){ const struct FakeID *id = id_;
  ...
  if (has(cls,"content/Context")){
    /* path getters -> our installation dir, so engine fopen()s land on the SD card */
