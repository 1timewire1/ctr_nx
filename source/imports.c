/* Dynamic-symbol resolution for the Android game libraries.
 *
 ...
 *
 * Module-to-module C++ symbols resolve from libc++_shared.so. This table supplies
 * the bionic/newlib shims, graphics, audio, Android NDK, and ABI helpers.
 */
...
struct zf_iovec { void *iov_base; size_t iov_len; };
...
static int android_puts_discard(const char *text) {
  (void)text;
  return 0;
}
...
int __android_log_print(int prio, const char *tag, const char *fmt, ...) {
  ...
  va_list ap;
  va_start(ap, fmt);
  const int result = vsnprintf(body, sizeof body, fmt, ap);
  va_end(ap);
  jni_offline_observe_log(tag, body);
  return result;
}
int __android_log_write(int prio, const char *tag, const char *text) {
  (void)prio;
  jni_offline_observe_log(tag, text);
...
int __android_log_vprint(int prio, const char *tag, const char *fmt, va_list va) {
  ...
  va_list copy;
  va_copy(copy, va);
  const int result = vsnprintf(body, sizeof body, fmt, copy);
  va_end(copy);
  jni_offline_observe_log(tag, body);
  return result;
}
...
/* newlib's pthread_detach() is an ENOSYS stub on Switch.  Returning that error
 * makes libc++'s std::thread::detach throw std::system_error, whose uncaught
 * path terminates the whole application during CTR's nativeInit.  Keep the
 * newlib pthread joinable internally, report Android-compatible detach success,
 * and non-blockingly join/free it once the kernel says it has exited. */
#define THREAD_RECORDS_MAX 256
typedef struct {
  pthread_t thread;
  ThreadStart *start;
  uint8_t detached;
...
static ThreadRecord g_thread_records[THREAD_RECORDS_MAX];
static pthread_mutex_t g_thread_records_lock = PTHREAD_MUTEX_INITIALIZER;

static void thread_record_add(pthread_t thread, ThreadStart *start, int detached) {
  pthread_mutex_lock(&g_thread_records_lock);
  for (int i = 0; i < THREAD_RECORDS_MAX; i++) {
    if (!g_thread_records[i].thread) {
      g_thread_records[i].thread = thread;
      g_thread_records[i].start = start;
      g_thread_records[i].detached = detached != 0;
      g_thread_records[i].reaping = 0;
      pthread_mutex_unlock(&g_thread_records_lock);
      return;
    }
  }
  pthread_mutex_unlock(&g_thread_records_lock);
  /* The thread remains valid even if diagnostics tracking is saturated. */
  debug_log("pthread: record table full; thread %p will remain joinable", (void *)thread);
}

static ThreadStart *thread_record_remove(pthread_t thread) {
  ThreadStart *start = NULL;
  pthread_mutex_lock(&g_thread_records_lock);
  for (int i = 0; i < THREAD_RECORDS_MAX; i++) {
    if (g_thread_records[i].thread == thread) {
      start = g_thread_records[i].start;
      memset(&g_thread_records[i], 0, sizeof(g_thread_records[i]));
      break;
    }
  }
  pthread_mutex_unlock(&g_thread_records_lock);
...
void pthread_compat_reap(void) {
  for (int i = 0; i < THREAD_RECORDS_MAX; i++) {
    pthread_t thread = NULL;
    pthread_mutex_lock(&g_thread_records_lock);
    if (g_thread_records[i].thread && g_thread_records[i].detached &&
        !g_thread_records[i].reaping) {
      Thread *nx_thread = (Thread *)g_thread_records[i].thread;
      s32 index = -1;
      const Handle handle = nx_thread->handle;
      if (handle != INVALID_HANDLE &&
          R_SUCCEEDED(svcWaitSynchronization(&index, &handle, 1, 0))) {
        g_thread_records[i].reaping = 1;
        thread = g_thread_records[i].thread;
      }
    }
    pthread_mutex_unlock(&g_thread_records_lock);
    if (thread) {
      /* pthread_join invokes libnx's threadClose and releases its owned stack. */
      pthread_join(thread, NULL);
      free(thread_record_remove(thread));
    }
  }
}

static void *thread_trampoline(void *p) {
  ThreadStart *ts = (ThreadStart *)p;   /* freed only after join: TPIDR points into tls */
...
int pthread_create_fake(pthread_t *thread, const void *bionic_attr, void *entry, void *arg) {
  ...
  const int detached = bionic_attr && ((const OurAttr *)bionic_attr)->magic == ATTR_MAGIC &&
                       ((const OurAttr *)bionic_attr)->detach != 0;
  thread_record_add(*thread, ts, detached);
  return 0;
}
int pthread_join_fake(pthread_t thread, void **retval) {
  const int r = pthread_join(thread, retval);
  if (r == 0) free(thread_record_remove(thread));
  return r;
}
int pthread_detach_fake(pthread_t thread) {
  ...
  for (int i = 0; i < THREAD_RECORDS_MAX; i++) {
    if (g_thread_records[i].thread == thread) {
      g_thread_records[i].detached = 1;
      break;
    }
  }
  pthread_mutex_unlock(&g_thread_records_lock);
  if (__sync_bool_compare_and_swap(&announced, 0, 1))
    debug_log("pthread: emulating Android detached threads (newlib detach is ENOSYS)");
  pthread_compat_reap();
  return 0;
}
...
static FakeKeySlot g_key_table[FAKE_KEYS_MAX];
...
static uint32_t next_key_generation(uint32_t generation) {
  generation++;
  return generation ? generation : 1;
}

static int fake_key_snapshot(unsigned index, uint32_t *generation,
                             void (**dtor)(void *)) {
  FakeKeySlot *slot = &g_key_table[index];
  if (!__atomic_load_n(&slot->used, __ATOMIC_ACQUIRE))
    return 0;
  const uint32_t current = __atomic_load_n(&slot->generation, __ATOMIC_ACQUIRE);
  if (generation)
    *generation = current;
  if (dtor)
    *dtor = __atomic_load_n(&slot->dtor, __ATOMIC_ACQUIRE);
  return __atomic_load_n(&slot->used, __ATOMIC_ACQUIRE) &&
         current == __atomic_load_n(&slot->generation, __ATOMIC_ACQUIRE);
}
...
static void master_key_dtor(void *p) {
  ...
  for (int iter = 0; iter < 4; iter++) {     // POSIX: rerun while dtors set new values
    ...
    for (int i = 0; i < FAKE_KEYS_MAX; i++) {
      ...
      void (*dtor)(void *) = NULL;
      if (v && fake_key_snapshot(i, &generation, &dtor) && dtor &&
          kv->generations[i] == generation) {
        kv->values[i] = NULL;
        kv->generations[i] = 0;
        dtor(v);
        again = 1;
      } else if (v && kv->generations[i] != generation) {
        /* The key was deleted/reused. POSIX does not run the old destructor,
         * and, critically, the new owner must never observe the stale value. */
        kv->values[i] = NULL;
        kv->generations[i] = 0;
      }
    }
...
int pthread_key_create_fake(unsigned *key, void (*dtor)(void *)) {
  ...
  for (unsigned i = 0; i < FAKE_KEYS_MAX; i++) {
    if (!__atomic_load_n(&g_key_table[i].used, __ATOMIC_RELAXED)) {
      const uint32_t old_generation =
          __atomic_load_n(&g_key_table[i].generation, __ATOMIC_RELAXED);
      const uint32_t generation = next_key_generation(old_generation);
      __atomic_store_n(&g_key_table[i].generation, generation, __ATOMIC_RELAXED);
      __atomic_store_n(&g_key_table[i].dtor, dtor, __ATOMIC_RELAXED);
      __atomic_store_n(&g_key_table[i].used, 1, __ATOMIC_RELEASE);
      *key = i + 1;                 // 1-based: a zeroed key is invalid
      if (old_generation > 0) {
        static int announced;
        if (__sync_bool_compare_and_swap(&announced, 0, 1))
          debug_log("pthread: fake TLS key reuse guarded by generations");
      }
      pthread_mutex_unlock(&g_key_mutex);
      return 0;
    }
...
int pthread_key_delete_fake(unsigned key) {
  ...
  pthread_mutex_lock(&g_key_mutex);
  FakeKeySlot *slot = &g_key_table[key - 1];
  if (!__atomic_load_n(&slot->used, __ATOMIC_RELAXED)) {
    pthread_mutex_unlock(&g_key_mutex);
    return EINVAL;
  }
  __atomic_store_n(&slot->used, 0, __ATOMIC_RELEASE);
  __atomic_store_n(&slot->dtor, NULL, __ATOMIC_RELAXED);
  const uint32_t generation =
      __atomic_load_n(&slot->generation, __ATOMIC_RELAXED);
  __atomic_store_n(&slot->generation, next_key_generation(generation),
                   __ATOMIC_RELEASE);
  pthread_mutex_unlock(&g_key_mutex);
  return 0;
}
...
void *pthread_getspecific_fake(unsigned key) {
  ...
  uint32_t generation = 0;
  if (!fake_key_snapshot(key - 1, &generation, NULL)) return NULL;
  KeyValues *kv = pthread_getspecific(g_master_key);
  return kv && kv->generations[key - 1] == generation
       ? kv->values[key - 1] : NULL;
}

int pthread_setspecific_fake(unsigned key, const void *value) {
  if (key == 0 || key > FAKE_KEYS_MAX || !g_master_key_ready) return EINVAL;
  uint32_t generation = 0;
  if (!fake_key_snapshot(key - 1, &generation, NULL)) return EINVAL;
  KeyValues *kv = pthread_getspecific(g_master_key);
  if (!kv) {
    kv = calloc(1, sizeof(*kv));
    if (!kv) return ENOMEM;
    const int result = pthread_setspecific(g_master_key, kv);
    if (result != 0) { free(kv); return result; }
  }
  kv->generations[key - 1] = generation;
...
/* Real networking: route fcntl/ioctl to the libnx BSD bridge so O_NONBLOCK / FIONBIO
 * on socket fds actually take effect (curl's non-blocking connect path needs this).
 * nx_* guard synthetic pipe fds and translate bionic constants -> newlib. */
extern int nx_fcntl(int fd, int cmd, int arg);
extern int nx_ioctl(int fd, unsigned long req, void *arg);
static int ioctl_stub(int fd, unsigned long req, ...) {
  va_list ap; va_start(ap, req); void *arg = va_arg(ap, void *); va_end(ap);
  return nx_ioctl(fd, req, arg);
}
static int fcntl_stub(int fd, int cmd, ...) {
  if (asset_pack_fd_is(fd)) {
    ...
    if (cmd == 1 /* F_GETFD */ || cmd == 2 /* F_SETFD */ ||
        cmd == 4 /* F_SETFL */)
      return 0;
    errno = EINVAL;
    return -1;
  ...
  int arg = 0;
  if (cmd != 1 /* F_GETFD */ && cmd != 3 /* F_GETFL */) {
    va_list ap; va_start(ap, cmd); arg = va_arg(ap, int); va_end(ap);
  }
  return nx_fcntl(fd, cmd, arg);
}
...
static DIR *opendir_fake(const char *path) {
  ...
  return packed ? (DIR *)packed : opendir(path);
}
...
static int fsync_impl(int fd) { return fsync(fd); }
...
static long pread_impl(int fd, void *buf, size_t n, long off) {
  return pread_fake(fd, buf, n, off);
}
static long pwrite_impl(int fd, const void *buf, size_t n, long off) {
  ...
  size_t total = 0;
  while (total < n) {
    long put = write(fd, (const uint8_t *)buf + total, n - total);
    if (put < 0) { if (!total) total = (size_t)-1; break; }
    if (put == 0) break;
    total += (size_t)put;
  }
  lseek(fd, cur, SEEK_SET);
  mutexUnlock(&lock);
  return total == (size_t)-1 ? -1 : (long)total;
}
...
static void NX_NORETURN abort_fake(void) {
  debug_log("runtime: Android abort requested");
  debug_shutdown();
  abort();
  __builtin_unreachable();
}
static void _exit_fake(int code) {
  debug_log("runtime: Android _exit(%d) requested", code);
  debug_shutdown();
...
/* Small Android/Bionic compatibility pieces used by ZFramework. */
static size_t __strlcpy_chk_fake(char *dst, const char *src, size_t size,
                                 size_t dst_size) {
  if (size > dst_size) abort();
  return strlcpy(dst, src ? src : "", size);
}
static void *__cmsg_nxthdr_fake(const void *msg, const void *cmsg) {
  (void)msg; (void)cmsg; return NULL;
}
static uint32_t arc4random_fake(void) { uint32_t value=0; randomGet(&value,sizeof value); return value; }
static uint32_t arc4random_uniform_fake(uint32_t upper) {
  if (upper < 2) return 0;
  uint32_t min = (uint32_t)(-upper) % upper, value;
  do { value = arc4random_fake(); } while (value < min);
  return value % upper;
}
static int getentropy_fake(void *buf, size_t size) { if (!buf && size) { errno=EFAULT; return -1; } randomGet(buf,size); return 0; }
static int epoll_create_fake(int size) { (void)size; errno=ENOSYS; return -1; }
static int epoll_create1_fake(int flags) { (void)flags; errno=ENOSYS; return -1; }
static int epoll_ctl_fake(int epfd,int op,int fd,void *ev) { (void)epfd;(void)op;(void)fd;(void)ev;errno=ENOSYS;return -1; }
static int epoll_wait_fake(int epfd,void *ev,int max,int timeout) { (void)epfd;(void)ev;(void)max;(void)timeout;errno=ENOSYS;return -1; }
static int eventfd_fake(unsigned initval,int flags) { (void)initval;(void)flags;errno=ENOSYS;return -1; }
static int socketpair_fake(int domain,int type,int protocol,int pair[2]) { (void)domain;(void)type;(void)protocol;return fakefd_pipe(pair); }
static long readv_fake_bridge(int fd,const struct zf_iovec *iov,int count) {
  long total=0; if(!iov||count<0){errno=EINVAL;return -1;} for(int i=0;i<count;i++){long n=read_fake(fd,iov[i].iov_base,iov[i].iov_len);if(n<0)return total?total:-1;total+=n;if((size_t)n<iov[i].iov_len)break;}return total;
}
static long writev_fake_bridge(int fd,const struct zf_iovec *iov,int count) {
  long total=0; if(!iov||count<0){errno=EINVAL;return -1;} for(int i=0;i<count;i++){long n=write_fake(fd,iov[i].iov_base,iov[i].iov_len);if(n<0)return total?total:-1;total+=n;if((size_t)n<iov[i].iov_len)break;}return total;
}
static int recvmmsg_fake(int fd,void *msg,unsigned count,int flags,void *timeout) { (void)fd;(void)msg;(void)count;(void)flags;(void)timeout;errno=ENOSYS;return -1; }
static int sendmmsg_fake(int fd,void *msg,unsigned count,int flags) { (void)fd;(void)msg;(void)count;(void)flags;errno=ENOSYS;return -1; }
static int fgetpos_bionic(FILE *file,void *pos) { if(!pos){errno=EINVAL;return -1;}long at=ftell_fake(file);if(at<0)return -1;*(int64_t*)pos=(int64_t)at;return 0; }
static int fsetpos_bionic(FILE *file,const void *pos) { if(!pos){errno=EINVAL;return -1;}return fseek_fake(file,(long)*(const int64_t*)pos,SEEK_SET); }
static int mlock_fake(const void *addr,size_t size) { (void)addr;(void)size;return 0; }
static int pthread_rwlock_destroy_bridge(void **rw) { if(rw&&*rw){free(*rw);*rw=NULL;}return 0; }
static long timezone_fake;
...
static void gl_CompileShader_host(GLuint shader) {
  glCompileShader(shader);
  GLint ok=GL_FALSE;
  glGetShaderiv(shader,GL_COMPILE_STATUS,&ok);
  if(!ok){
    char info[1024]={0};GLsizei length=0;
    glGetShaderInfoLog(shader,sizeof info,&length,info);
    debug_log("gfx: shader %u compile failed: %s",shader,info);
  }
}
static void gl_LinkProgram_host(GLuint program) {
  glLinkProgram(program);
  GLint ok=GL_FALSE;
  glGetProgramiv(program,GL_LINK_STATUS,&ok);
  if(!ok){
    char info[1024]={0};GLsizei length=0;
    glGetProgramInfoLog(program,sizeof info,&length,info);
    debug_log("gfx: program %u link failed: %s",program,info);
  }
}
...
/* Minimal NDK Choreographer used by Unity's frame loop. */
typedef void (*achoreo_cb32)(long, void *);
typedef void (*achoreo_cb64)(int64_t, void *);
static struct { void *cb, *data; int is64; } g_choreo_cbs[32];
static pthread_mutex_t g_choreo_lock = PTHREAD_MUTEX_INITIALIZER;
static void *AChoreographer_getInstance_fake(void) {
  return (void *)0x4143484f; /* 'OCHA' non-NULL */
}
static void achoreo_post(void *cb, void *d, int is64) {
  pthread_mutex_lock(&g_choreo_lock);
  for (int i = 0; i < 32; i++) if (!g_choreo_cbs[i].cb) { g_choreo_cbs[i].cb = cb; g_choreo_cbs[i].data = d; g_choreo_cbs[i].is64 = is64; break; }
  pthread_mutex_unlock(&g_choreo_lock);
}
static void  AChoreographer_postFrameCallback_fake(void *c, void *cb, void *d)   { (void)c; achoreo_post(cb, d, 0); }
static void  AChoreographer_postFrameCallback64_fake(void *c, void *cb, void *d) { (void)c; achoreo_post(cb, d, 1); }
/* invoked by the clock thread (main.c) ~each vsync with the current monotonic time */
void choreographer_tick(int64_t now_ns) {
  void *cbs[32], *datas[32]; int is64s[32], n = 0;
  pthread_mutex_lock(&g_choreo_lock);
  for (int i = 0; i < 32; i++) if (g_choreo_cbs[i].cb) {
    cbs[n] = g_choreo_cbs[i].cb; datas[n] = g_choreo_cbs[i].data; is64s[n] = g_choreo_cbs[i].is64; n++;
    g_choreo_cbs[i].cb = NULL;
  }
  pthread_mutex_unlock(&g_choreo_lock);
  for (int i = 0; i < n; i++) {
    if (is64s[i]) ((achoreo_cb64)cbs[i])(now_ns, datas[i]);
    else          ((achoreo_cb32)cbs[i])((long)now_ns, datas[i]);
  }
}
static void *ANativeWindow_toSurface_fake(void *env, void *win) { (void)env; (void)win; return NULL; }
static int   ALooper_removeFd_fake(void *l, int fd) { (void)l; (void)fd; return 0; }
static long  process_vm_readv_fake(int pid, const void *lv, unsigned long lc,
                                   const void *rv, unsigned long rc, unsigned long f) {
  (void)pid; (void)lv; (void)lc; (void)rv; (void)rc; (void)f; return -1;  /* not supported */
}
static char *__gnu_strerror_r_fake(int e, char *buf, size_t n) { (void)e; if (buf && n) buf[0] = 0; return buf; }
extern char **environ;   /* libil2cpp reads it; newlib provides a valid (empty) char** */
...
DynLibFunction dynlib_functions[] = {
  ...
  { "__system_property_find", (uintptr_t)&__system_property_find_fake },   // Swappy API-level gate -> AChoreographer
  ...
  { "arc4random_uniform", (uintptr_t)&arc4random_uniform_fake },
  { "getentropy", (uintptr_t)&getentropy_fake },
  ...
  // --- printf family ---
  { "printf", (uintptr_t)&android_printf_discard }, { "puts", (uintptr_t)&android_puts_discard },
  ...
  { "cbrt", (uintptr_t)&cbrt }, { "frexp", (uintptr_t)&frexp },
  { "scalbnf", (uintptr_t)&scalbnf },
  { "fmodf", (uintptr_t)&fmodf }, { "sincosf", (uintptr_t)&sincosf_fake },
  // --- additional libm + libc ---
  { "exp2", (uintptr_t)&exp2 }, { "tanhf", (uintptr_t)&tanhf },
  { "hypot", (uintptr_t)&hypot }, { "hypotf", (uintptr_t)&hypotf },
  { "remainder", (uintptr_t)&remainder }, { "remainderf", (uintptr_t)&remainderf },
  { "nextafter", (uintptr_t)&nextafter }, { "nextafterf", (uintptr_t)&nextafterf },
  { "strncat", (uintptr_t)&strncat },
  { "fseeko64", (uintptr_t)&fseeko_fake }, { "ftello64", (uintptr_t)&ftello_fake },
  // --- NDK Choreographer/window + misc ---
  { "AChoreographer_getInstance", (uintptr_t)&AChoreographer_getInstance_fake },
  { "AChoreographer_postFrameCallback", (uintptr_t)&AChoreographer_postFrameCallback_fake },
  { "AChoreographer_postFrameCallback64", (uintptr_t)&AChoreographer_postFrameCallback64_fake },
  { "ANativeWindow_toSurface", (uintptr_t)&ANativeWindow_toSurface_fake },
  { "ALooper_removeFd", (uintptr_t)&ALooper_removeFd_fake },
  { "process_vm_readv", (uintptr_t)&process_vm_readv_fake },
  { "__gnu_strerror_r", (uintptr_t)&__gnu_strerror_r_fake },
  { "environ", (uintptr_t)&environ }, { "msync", (uintptr_t)&msync_fake },  // libil2cpp's 2 imports

  // --- time ---
  { "clock_gettime", (uintptr_t)&clock_gettime_bionic }, { "gettimeofday", (uintptr_t)&gettimeofday },
  ...
  { "sleep", (uintptr_t)&sleep }, { "timezone", (uintptr_t)&timezone_fake },
  ...
  { "fopen", (uintptr_t)&fopen_fake }, { "fdopen", (uintptr_t)&fdopen_fake },
  { "fclose", (uintptr_t)&fclose_fake },
  { "fread", (uintptr_t)&fread_fake }, { "fwrite", (uintptr_t)&fwrite_fake },
  { "fseek", (uintptr_t)&fseek_fake }, { "fseeko", (uintptr_t)&fseeko_fake },
  { "ftell", (uintptr_t)&ftell_fake }, { "ftello", (uintptr_t)&ftello_fake },
  { "fgetpos", (uintptr_t)&fgetpos_bionic }, { "fsetpos", (uintptr_t)&fsetpos_bionic },
  ...
  { "freopen", (uintptr_t)&freopen }, { "mkstemp", (uintptr_t)&mkstemp },

  // --- zlib (Wwise sound-bank decompression) ---
  ...
  { "readv", (uintptr_t)&readv_fake_bridge }, { "writev", (uintptr_t)&writev_fake_bridge },
  { "eventfd", (uintptr_t)&eventfd_fake },
  { "epoll_create", (uintptr_t)&epoll_create_fake }, { "epoll_create1", (uintptr_t)&epoll_create1_fake },
  { "epoll_ctl", (uintptr_t)&epoll_ctl_fake }, { "epoll_wait", (uintptr_t)&epoll_wait_fake },
  ...
  { "recvmmsg", (uintptr_t)&recvmmsg_fake }, { "sendmmsg", (uintptr_t)&sendmmsg_fake },
  ...
  { "pthread_rwlock_destroy", (uintptr_t)&pthread_rwlock_destroy_bridge },
  ...
  { "eglSwapBuffers", (uintptr_t)&egl_SwapBuffers_host }, { "eglQuerySurface", (uintptr_t)&egl_QuerySurface_host },
  ...
  { "glCompileShader", (uintptr_t)&gl_CompileShader_host }, { "glCompressedTexImage2D", (uintptr_t)&glCompressedTexImage2D },
  ...
  { "glGetActiveAttrib", (uintptr_t)&glGetActiveAttrib }, { "glGetActiveUniform", (uintptr_t)&glGetActiveUniform },
  { "glGetBooleanv", (uintptr_t)&glGetBooleanv }, { "glGetFloatv", (uintptr_t)&glGetFloatv },
  ...
  { "glGetShaderInfoLog", (uintptr_t)&glGetShaderInfoLog }, { "glGetString", (uintptr_t)&glGetString },
  { "glIsEnabled", (uintptr_t)&glIsEnabled }, { "glLineWidth", (uintptr_t)&glLineWidth },
  { "glLinkProgram", (uintptr_t)&gl_LinkProgram_host }, { "glPixelStorei", (uintptr_t)&glPixelStorei },
  ...
  { "glShaderSource", (uintptr_t)&gl_ShaderSource_host }, { "glStencilFunc", (uintptr_t)&glStencilFunc },
  ...
  { "glUniform1f", (uintptr_t)&glUniform1f }, { "glUniform1i", (uintptr_t)&glUniform1i },
  { "glUniform1iv", (uintptr_t)&glUniform1iv }, { "glUniform2fv", (uintptr_t)&glUniform2fv },
  { "glUniform3fv", (uintptr_t)&glUniform3fv }, { "glUniform4fv", (uintptr_t)&glUniform4fv },
  { "glUniform4f", (uintptr_t)&glUniform4f }, { "glUniformMatrix3fv", (uintptr_t)&glUniformMatrix3fv },
  { "glUniformMatrix4fv", (uintptr_t)&glUniformMatrix4fv }, { "glUseProgram", (uintptr_t)&glUseProgram },
  { "glVertexAttrib1f", (uintptr_t)&glVertexAttrib1f }, { "glVertexAttrib2fv", (uintptr_t)&glVertexAttrib2fv },
  { "glVertexAttrib3fv", (uintptr_t)&glVertexAttrib3fv }, { "glVertexAttrib4f", (uintptr_t)&glVertexAttrib4f },
  ...
  // --- AAudio (aaudio.c) -- Wwise resolves these through dlopen/dlsym ---
  ...
  { "AAudioStreamBuilder_setContentType", (uintptr_t)&AAudioStreamBuilder_setContentType },
  { "AAudioStreamBuilder_setUsage", (uintptr_t)&AAudioStreamBuilder_setUsage },
  ...
  { "AAudioStream_getFramesPerDataCallback", (uintptr_t)&AAudioStream_getFramesPerDataCallback },
...
// Additional Unity/IL2CPP libc imports
...
static void build_combined(void) {
  ...
  g_combined_n = (int)dynlib_numfunctions + unity_dynlib_numfunctions
               + (int)firebase_extra_numfunctions + (int)android_media_numfunctions;
  ...
  memcpy(g_combined + off, android_media_functions,
         android_media_numfunctions * sizeof(DynLibFunction));
}
...
void resolve_imports(so_module *mod) {
  so_relocate(mod);
  /* Search the base and Unity tables in one pass before unresolved symbols are
   * replaced with failure stubs. */
  build_combined();
  int missing = so_resolve(mod, g_combined, g_combined_n, 1);
  if (missing < 0)
    fatal_error("%s has %d unresolved Android imports. See debug.log for their names.",
                mod->name, -missing);
}
